...
async def no_step(content, author, event):
    data=content.split('-')
    new_event = Event(id=0, gameName=data[1], slots=int(data[2]), time=data[3], authorId=author.id, players=author.display_name, role=data[4], step=res.steps['done'], players_id=str(author.id))
